...
// Computed: Gambar yang sedang aktif berdasarkan index
const selectedImage = computed(() => {
	if (selectedIndex.value === -1) return null;
	return filteredItems.value[selectedIndex.value] as any;
});
...
const nextImage = () => {
	if (selectedIndex.value < filteredItems.value.length - 1) {
		...
		selectedIndex.value = 0; // Loop ke awal
	}
};
...
const prevImage = () => {
	if (selectedIndex.value > 0) {
		...
		selectedIndex.value = filteredItems.value.length - 1; // Loop ke akhir
	}
};
